...
import { GradesService } from 'src/modules/grade-book/grades/grades.service'
...
import { LessonsTypeRu } from 'src/modules/grade-book/grade-book/entities/grade-book.entity'
...
import { RemoveLessonsFromStreamDto } from 'src/modules/core/streams/dto/remove-lessons-from-stream.dto'
...
export class GroupLoadLessonsService {
  ...
  async findAllByGroupId(groupId: number) {
    ...
    if (!lessons.length) {
      ...
      if (group) {
        throw new NotFoundException(`Дисципліни групи: ${group.name} не знайдені`)
      }
